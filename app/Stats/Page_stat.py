...
class Graph_1_exe(App_stat):
    def __init__(self, master, user_name, x0, y0, x1, title, Legend1, Legend2, name_y_axe):
        ...
        if x1 == []:
            rects1 = ax.bar(ind, x0, width, label=Legend1)

        else:
            rects1 = ax.bar(ind - width / 2, x0, width, label=Legend1)
            rects2 = ax.bar(ind + width / 2, x1, width, label=Legend2)
...
class Graph_4_exe(App_stat):
    def __init__(self, master, user_name, score_app, game, x1, title, title2, Legend1, Legend2, name_y_axe):
        ...
        wedges, texts, autotexts = ax.pie(
            ...
            explode=explode,
            labels=game,
            autopct=lambda pct: func(pct, score_app),
            colors=["red", "green", "yellow", "blue", "orange", "pink", "lightgrey", "brown", "purple"],
            pctdistance=0.7,
            labeldistance=1.2,
            shadow=True,
            startangle=90,
