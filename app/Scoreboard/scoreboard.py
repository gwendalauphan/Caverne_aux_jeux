...
class Scoreboard:
    def __init__(self, parent, root, jeux, User_name, posx=60, posy=45):
        ...
        Label(TopFrame, text="Rang" + " " * 8 + "Nom" + " " * 28 + "Score    ", font=("Helvetica", 15), relief=GROOVE).place(
            x=70, y=90
        )  # légende
        ...
        for i in range(10 - len(players)):  # si jamais la liste est plus petite que 10, on affiche des emplacements vides
            Label(TopFrame, text="#{} :".format(i + 1 + len(players)), font=("Helvetica", 15)).place(
                x=85, y=120 + 25 * len(players) + i * 26
            )
