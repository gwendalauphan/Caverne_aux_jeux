...
class pong:
    def __init__(self, user):
        ...
        self.image1 = PhotoImage(file=resource_path("Pong/res/first_image.png"))
        self.image2 = PhotoImage(file=resource_path("Pong/res/second_image.png"))
        self.image3 = PhotoImage(file=resource_path("Pong/res/third_image.png"))

        self.Frame_main1_wind2 = Canvas(self.show_rules, bg="white", relief=GROOVE)
        self.Frame_main1_wind2.pack(ipadx=670, ipady=526)
        self.Fond_Frame_main1_wind2 = PhotoImage(file=resource_path("Pong/res/regles_pong.png"))
        ...
        second_label = Label(
            self.Frame_main2_wind2, text="La balle peut aussi rebondir sur les murs, \n il te faut alors prévoir sa trajectoire"
        )
        ...
        ##########-----------Import des photos------------------###############################
        self.raquette_pong = PhotoImage(file=resource_path("Pong/res/raquette_pong.png"))
        self.button_start_image = PhotoImage(file=resource_path("Pong/res/button_start.png"))
        self.title_pong = PhotoImage(file=resource_path("Pong/res/title_pong.png"))
        self.femme_pong = PhotoImage(file=resource_path("Pong/res/femme_pong.png"))
    ...
    def update(self):
        ...
        self.Canvas_dessine.create_text(
            430, 35, text="Meilleur score par {}:".format(self.Best_Player[0]), fill="white", font=("Helvetica", 10)
        )

        best = str(int(self.Best_Player[1] / 50))
        self.bestUnit = PhotoImage(file=resource_path("Pong/res/" + best[-1] + ".png"))
        self.Canvas_dessine.create_image(435, 75, image=self.bestUnit)
        if len(best) > 1:
            self.bestDix = PhotoImage(file=resource_path("Pong/res/" + best[-2] + ".png"))
            self.Canvas_dessine.create_image(410, 75, image=self.bestDix)
            if len(best) > 2:
                self.bestCent = PhotoImage(file=resource_path("Pong/res/" + best[-3] + ".png"))
                self.Canvas_dessine.create_image(385, 75, image=self.bestCent)

        temp = str(self.touch)
        self.scoreUnit = PhotoImage(file=resource_path("Pong/res/" + temp[-1] + ".png"))
        self.Canvas_dessine.create_image(300, 75, image=self.scoreUnit)
        if len(temp) > 1:
            self.scoreDix = PhotoImage(file=resource_path("Pong/res/" + temp[-2] + ".png"))
            self.Canvas_dessine.create_image(275, 75, image=self.scoreDix)
            if len(temp) > 2:
                self.scoreCent = PhotoImage(file=resource_path("Pong/res/" + temp[-3] + ".png"))
...
class Ball:
    ...
    def update(self):
        ...
        if self.pos.x + 20 > self.parent.width - 20:  # Gagner
