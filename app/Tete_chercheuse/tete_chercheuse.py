...
class application:
    def __init__(self, user):
        ...
        ########---------Import Photos------------------###############################################
        self.robot = [
            PhotoImage(file=resource_path("Tete_chercheuse/image/robot_right.png")),
            PhotoImage(file=resource_path("Tete_chercheuse/image/robot_front.png")),
            PhotoImage(file=resource_path("Tete_chercheuse/image/robot_left.png")),
            PhotoImage(file=resource_path("Tete_chercheuse/image/robot_back.png")),
        ]

        self.restart_button_image = PhotoImage(file=resource_path("Tete_chercheuse/image/start.png"))
        self.Flag = PhotoImage(file=resource_path("Tete_chercheuse/image/flag.png"))
        self.End = PhotoImage(file=resource_path("Tete_chercheuse/image/robot_flag.png"))
        self.Caisse = PhotoImage(file=resource_path("Tete_chercheuse/image/caisse.png"))
        self.Wall = PhotoImage(file=resource_path("Tete_chercheuse/image/mur.ppm"))
        self.Fond_Frame_main1_wind2 = PhotoImage(file=resource_path("thumbnail/Tete2.png"))
        self.Yellow_Coin = PhotoImage(file=resource_path("Tete_chercheuse/image/yellow_coin.ppm"))
        self.Red_Coin = PhotoImage(file=resource_path("Tete_chercheuse/image/red_coin.ppm"))
        self.Guide_tete = PhotoImage(file=resource_path("Tete_chercheuse/image/brain.png"))
        self.Bulle_image = PhotoImage(file=resource_path("Tete_chercheuse/image/bulle.png"))
        ...
        self.Button_restart = Button(
            self.Frame_top, text="RESTART", relief=GROOVE, cursor="hand2", font=("Helvetica", 10), command=self.restart_button
        )
    ...
    def rules_game(self):
        ...
        self.first_photo1 = PhotoImage(file=resource_path("Tete_chercheuse/image/first_photo.png"))  # import de l'image
        ...
        Rules2 = Label(
            ...
            text="Pour cela, tu as à disposition des caisses \n\
        qui te permettront de dévier le robot.",
            font=("Berlin Sans FB", 12),
        )
        self.Frame_main2_wind2.after(2000, lambda: Rules2.place(x=40, y=150))

        self.first_photo2 = PhotoImage(file=resource_path("Tete_chercheuse/image/second_photo.png"))
        ...
        Rules3 = Label(
            self.Frame_main2_wind2,
            text="A chaque fois que le robot rencontre un obstacle,\n il tourne à SA droite sinon il va toujours tout droit.",
            font=("Berlin Sans FB", 12),
        )
        self.Frame_main2_wind2.after(3500, lambda: Rules3.place(x=20, y=240))

        self.first_photo3 = PhotoImage(file=resource_path("Tete_chercheuse/image/third_photo.png"))
        ...
        Rules4 = Label(
            self.Frame_main2_wind2,
            text="Enfin tu peux rencontrer des pièces rouges et \n\
        jaunes, en passant dessus tu gagneras des points. \n\
        Les rouges rapportent plus de points que les jaunes.",
            font=("Berlin Sans FB", 12),
        )
        self.Frame_main2_wind2.after(5000, lambda: Rules4.place(x=17, y=320))

        self.first_photo4 = PhotoImage(file=resource_path("Tete_chercheuse/image/four_photo.png"))
        ...
        # --------------5------------------------------------
        self.Bouton_skip = Button(
            self.Frame_main2_wind2, text="-Skip-", font=("Helvetica", 10), cursor="hand2", relief=GROOVE, command=self.quit_rules
        )
            ...
            for j in range(10):
                if (self.table[i][j]) == "X":
                    self.Table.create_image(
                        self.cell_width * i + self.cell_width / 2, self.cell_height * j + self.cell_height / 2, image=self.Wall
                    )
                elif self.table[i][j] == "C":
                    self.Table.create_image(
                        self.cell_width * i + self.cell_width / 2, self.cell_height * j + self.cell_height / 2, image=self.Caisse
                    )
                elif (self.table[i][j]) == "R":
                    self.Table.create_image(
                        self.cell_width * i + self.cell_width / 2,
                        self.cell_height * j + self.cell_height / 2,
                        image=self.robot[self.index_robot],
                    )
                elif (self.table[i][j]) == "P":
                    self.Table.create_image(
                        self.cell_width * i + self.cell_width / 2, self.cell_height * j + self.cell_height / 2, image=self.Flag
                    )
                elif (self.table[i][j]) == "S":
                    self.Table.create_image(
                        self.cell_width * i + self.cell_width / 2, self.cell_height * j + self.cell_height / 2, image=self.Yellow_Coin
                    )
                elif (self.table[i][j]) == "B":
                    self.Table.create_image(
                        self.cell_width * i + self.cell_width / 2, self.cell_height * j + self.cell_height / 2, image=self.Red_Coin
                    )
                elif self.table[i][j] == "E":
                    self.Table.create_image(
                        self.cell_width * i + self.cell_width / 2, self.cell_height * j + self.cell_height / 2, image=self.End
                    )
    ...
    def next(self):
        ...
        self.score.append(
            50 * (self.level + 1)
        )  # création d'un nouveau slot dans la liste des scores pour le nouveau niveau initié avec une valeur car il y a un malus appliqué dans la foction restart
    ...
    def restart_question(self):
        ...
        answer1 = Button(
            self.canvas_question, text=" Oui ", command=self.restart2, cursor="hand2", fg="white", bg="black", font=("Helvetica", 12)
        ).place(x=120, y=150)
        answer2 = Button(
            self.canvas_question, text=" Non ", command=self.end_game, cursor="hand2", fg="white", bg="black", font=("Helvetica", 12)
        ).place(x=300, y=150)
...
def Tete(user):  # fonction principale
    ...
    if jeux.count != 0:
        return (
            sum(jeux.score),
            sum(jeux.score) / jeux.count,
            (time() - jeux.time_start) / jeux.count,
            jeux.count,
            [],
        )  # retour du score total du joueur sur tout les niveaux joués
    else:
        return (0, 0, 0, 0, [])
