...
class demineur:
    def __init__(self, user):
        ...
        # import des images
        self.Number_Image = [
            PhotoImage(file=resource_path("Minesweeper/Images/0.png")).subsample(8),
    ...
    def start(self, level):  # fontion appelée après la sélection de la difficulté avec level en paramètre
        ...
        self.canvas = Canvas(
            self.Frame_right, width=self.dims[0] * self.border, height=self.dims[1] * self.border, bg="red", highlightthickness=0
        )
        ...
        for i in range(self.dims[0]):
            for j in range(self.dims[1]):  # on initialise la grille avec des images de bouton
                self.list_images[i][j] = self.canvas.create_image(
                    i * self.border + self.border / 2, j * self.border + self.border / 2, image=self.Normal_Image
                )
    ...
    def click(self, event):  # fonction déclanchée avec un clique de la souris sur le canvas
        ...
        if event.num == 1:  # si clique gauche
            ...
            if self.canvas.itemconfigure(self.list_images[x][y])["image"][-1] != str(self.Normal_Image) and self.canvas.itemconfigure(
                self.list_images[x][y]
            )["image"][-1] != str(self.Flag_Image):
            ...
            if self.canvas.itemconfigure(self.list_images[x][y])["image"][-1] == str(self.Flag_Image):
                ...
                self.canvas.itemconfigure(self.list_images[x][y])["image"][-1] == str(self.Normal_Image)
                and self.flag_count < self.mine_Count
            ):
