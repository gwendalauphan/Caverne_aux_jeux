...
def init_ranking_rows(parent):
    ...
    for i in range(10):
        lp = Label(parent, text=f"#{i+1} :", bg="#111111", foreground="#00e600", font=("Helvetica", 8))
        lp.place(x=4, y=75 + i * 20)
        ls = Label(parent, text="", bg="#111111", foreground="#00e600", font=("Helvetica", 8))
...
class BoutonS:
    ...
    def __init__(self, app: "App", canvas: tk.Canvas, x, y, jeux, run, name):
        ...
        canvas.create_text(y * (450 / 5) + 20, x * (265 / 4) - 90, text=name, font=("Berlin Sans FB", 20), fill="white")
...
class App:
    ...
    def _build_main(self):
        ...
        self.Frame_main = Canvas(root, highlightthickness=0, borderwidth=2, bg="#111111", relief=GROOVE, scrollregion=(0, 0, 250, 1000))
        ...
        def _on_mousewheel_linux_up(event):
            self.Frame_main.yview_scroll(-1, "units")
        ...
        self.Button_para = Button(
            self.Frame_top, image=self.gearImg, bg="#111111", borderwidth=0, highlightthickness=0, cursor="hand2", command=self.para
        )
        self.Button_para.place(x=860, y=25)

        Button(
            self.Frame_top, image=self.image_stat, bg="#111111", borderwidth=0, highlightthickness=0, cursor="hand2", command=self.execute
        ).place(x=930, y=20)
        ...
        # Refresh button
        Button(
            self.Frame_left, text="Rafraîchir", cursor="hand2", command=self.refresh_ranking, bg="#1e1e1e", fg="#00e600", relief=GROOVE
        ).place(x=65, y=350)
        ...
        # Friendly footer
        Label(
            self.Frame_down, text=f"Bienvenue, {self.user_name} !", pady=10, bg="#111111", fg="#00e600", font=("Helvetica", 10, "italic")
        ).pack()
    ...
    def para(self):
        ...
        text = (
            "Le but de cette application est de s'amuser en jouant à des jeux.\n"
        ...
        Label(self.playground, text="Aide", font=("Helvetica", 25), bg="#111111", fg="#888888").place(x=300, y=20)
        Label(self.playground, text=text, font=("Helvetica", 10), bg="#111111", fg="#888888").place(x=100, y=60)
