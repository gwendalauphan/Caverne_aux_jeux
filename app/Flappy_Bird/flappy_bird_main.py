...
from Scoreboard.scoreboard import get_game_score_list, Scoreboard
...
class Bird: # pylint: disable=too-many-arguments
    def __init__(self, user):
        ...
        for i in range(2, 31):
            self.liste_image.append(PhotoImage(file=resource_path(f"Flappy_Bird/Ressources/{i}.png")))

        self.background = []  # Import des photos du décor mis au hasard
        for k in range(6):
            self.background.append(PhotoImage(file=resource_path(f"Flappy_Bird/Ressources/decor{k + 1}.png")))

        self.liste_nombres = []  # Import des photos des nombres pour le score
        for p in range(10):
            self.liste_nombres.append(PhotoImage(file=resource_path(f"Flappy_Bird/Ressources/u{p}.png")))
    ...
    def build_game(self):  # Fonction servant au lancement du jeu (appellée à chaque restart) #Création de la map
        ...
        self.Frame_main_game.create_text(
            100, 250, text="Attention aux tuyaux !!!! ", fill="white", font=("Berlin Sans FB", FONT_SIZE_SMALL)
        )
        ...
        self.Frame_main_game.create_text(95, 460, text="A savoir...", fill="white", font=("Berlin Sans FB", FONT_SIZE_SMALL))
        self.Frame_main_game.create_text(
            95, 490, text="Une grosse surprise\n t'attend à la fin", fill="white", font=("Berlin Sans FB", FONT_SIZE_MEDIUM)
        )

        bestplayer = get_game_score_list("Flappy")[0]
        self.Frame_main_game.create_text(85, 120, text="Meilleur joueur:", fill="white", font=("Berlin Sans FB", FONT_SIZE_MEDIUM))
        self.Frame_main_game.create_text(
            85, 135, text="{} points".format(int(bestplayer[1] / 100)), fill="white", font=("Berlin Sans FB", FONT_SIZE_MEDIUM)
        )
        ...
        self.show_time = Label(
            self.canvas_show_time, text="Temps: %s" % str(0), foreground="white", bg="black", font=("Berlin Sans FB", FONT_SIZE_BIG)
        )
        ...
        self.image_Bird = self.Canvas_world.create_image(
            350, 220, image=self.liste_image[int(self.count_image)]
        )  # Image de l'oiseau dans l'attente
    ...
    def start(self):  # Fonction servant à créer le nouvel oiseau qui pourra voler et les 4 tuyaux
        ...
        self.image_Bird_true = self.Canvas_world.create_image(
            125, 220, image=self.liste_image[int(self.count_image)]
        )  # Création du nouvel oiseau
    ...
    def bird_move(self):  # Fonction servant à déplacer le sol et l'oiseau pendant le jeu
        ...
        for _ in range(6):  # Début de la boucle pour plus de calculs et de précision
            if self.press is True:  # La montée de l'oiseau
                ...
                self.x_center_bird, self.y_center_bird = self.Canvas_world.coords(
                    self.image_Bird_true
                )  # Coordonées de l'oiseau qui servent ensuite à la fonction verif_bird
                self.Canvas_world.move(self.image_Bird_true, self.x, self.y)  # Déplacement de l'oiseau
                self.Canvas_world.itemconfigure(
                    self.image_Bird_true, image=self.liste_image[int(self.count_image)]
                )  # Changement de l'inclinaison de l'oiseau
                ...
                if self.count_image < 25:  # Si self.count_image < 27, pour éviter index out of range
                    ...
                    self.x_center_bird, self.y_center_bird = self.Canvas_world.coords(
                        self.image_Bird_true
                    )  # On récupérer les coordonnées de l'oiseau
                    ...
                    if (self.y_center_bird + self.vitesse + 25) > 500:  # Grace aux coordonées, on vérifie si l'oiseau ne touche pas le sol
                        ...
                        self.verif_bird(0, 0, dead=True)  # appelle la fonction verif_bird avec dead = True (IMPORTANT), cela sert à
                        # ne pas lancer la fonction wait_dead ensuite ainsi que l'arret de la fonction update
                    else:  # Sinon:
                        self.Canvas_world.move(
                            self.image_Bird_true, self.x, self.vitesse
                        )  # On déplace l'oiseau et on change son inclinaison avec itemconfigure
                    ...
                    if (self.y_center_bird + self.vitesse + 25) > 500:  #
                        ...
                        self.verif_bird(0, 0, dead=True)  # Même résonnement
                    else:  #
    ...
    def verif_bird(
        self, y_pipe_center_top, y_pipe_center_down, dead=False
    ):  # Fonction serant à vérifier si l'oiseau rentre en contact avec le tuyau ou non
        if self.verite:  # Cette condition sert à n'executer qu'une fois cette fonction car après l'appel de celle-ci, self.verite = False
            ...
            if (
                self.y_pipe_down < self.y_center_bird + 24 or self.y_center_bird - 24 < self.y_pipe_top or dead
            ):  # Si il y a touche du sol ou du tuyau
                ...
                if dead:  # Cette condition sert à différencier
                    self.root.after(2000, self.dead)  # si la mort vient du sol ou du tuyau
                else:  # ainsi si dead == True, on n'appelle pas wait_dead puique l'oiseau est déjà au sol
    ...
    def wait_dead(self):  # Fonction servant à faire descendre l'oiseau lorsque ce dernier a touché un tuyau
        ...
        if y + self.vitesse + 25 < 500:  # Condition pour savoir si il a atteint le sol ou non
            ...
            if (
                self.count_image < 25
            ):  # Si self.count_image < 250, on continue à itemconfigure avec image = self.liste_image[int(abs(self.count_image)/10)]
    ...
    def dead(self):
        ...
        if self.y_center_bird < 0:
            a = 0
        else:
            a = self.y_center_bird / 475
        self.death_pos.append((9, int(a * 19)))
...
class Pipe:  # Creéation de la classe Pipe
    ...
    def move_pipe(self):  # Fonction servant à déplacer le tuyau à l'horizontal, la fonction tourne toutes les 50ms, grace à update
        ...
        self.x_center_down_pipe, self.y_center_down_pipe = self.parent.Canvas_world.coords(
            self.down_pipe
        )  # tuyaux qui serviront à verif_pipe
    ...
    def verif_pipe(self):  # Fonction servant à vérifier si le tuyau dans des intervalles donnés
        if self.x_center_top_pipe + 65 < 0:  # Si le tuyau a quitté l'écran, on le supprime et on le recréer directement caché à droite
            ...
            self.create_pipe(
                randint(1425, 1475), randint(100, 400)
            )  # Création du nouveau tuyau, les randint servent à donner plus de fun et de difficultés au jeu

        if 40 < self.x_center_top_pipe < 210:  # Si le tuyau se trouve entre 100 et 150 pixels (c'est à dire au niveau de l'oiseau)
            self.parent.verif_bird(
                self.y_center_top_pipe, self.y_center_down_pipe
            )  # On appelle la fonction verif_bird permettant de voir si il y contact ou non

        if (
            100 >= self.x_center_top_pipe + 60 > 98
        ):  # Si le tuyau dépassé l'oiseau, l'interval est de 12 pixels car le tuyau se déplaces de 12 lui aussi toules 50ms
